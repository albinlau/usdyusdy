    ...
    IStabilityPoolEvents
{
    ...
    function withdrawFromSP(uint256 _amount, bool _doClaim) external override {
        ...
        uint256 newDeposit = compoundedUSDXDeposit -
            usdxToWithdraw +
            keptYieldGain;
        (
            uint256 newStashedColl,
        ...
        _updateDepositAndSnapshots(msg.sender, newDeposit, newStashedColl);
        _decreaseYieldGainsOwed(currentYieldGain);
        uint256 newTotalUSDXDeposits = _updateTotalUSDXDeposits(
            keptYieldGain,
            usdxToWithdraw
        );
        ...
        require(
            newTotalUSDXDeposits >= MIN_USDX_IN_SP,
            "Withdrawal must leave totalUSDXDeposits >= MIN_USDX_IN_SP"
        );
    }
    ...
    function _updateYieldRewardsSum(uint256 _newYield) internal {
        ...
        scaleToB[currentScale] +=
            (P * accumulatedYieldGains) /
            totalUSDXDeposits;
    ...
    function offset(
        ...
        // Thus, to turn `P` zero, `totalUSDXDeposits` has to be greater than `1e27 * 1e18`,
    ...
    function _updateTotalUSDXDeposits(
        uint256 _depositIncrease,
        uint256 _depositDecrease
    ) internal returns (uint256) {
        if (_depositIncrease == 0 && _depositDecrease == 0)
            return totalUSDXDeposits;
        uint256 newTotalUSDXDeposits = totalUSDXDeposits +
            _depositIncrease -
            _depositDecrease;
    ...
    function getCompoundedUSDXDeposit(
        address _depositor
    ) public view override returns (uint256 compoundedDeposit) {
        uint256 initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) return 0;
