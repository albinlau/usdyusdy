    ...
    function toLocaleString(
        ...
        if (_divisor > 0) {
            ...
            if (_divisor <= _precision) {
                ...
                fraction = (whole == 0 && _value != 1)
                    ? fraction * 10
                    : fraction;
            } else {
                fraction =
                    (_value % 10 ** _divisor) /
                    10 ** (_divisor - _precision - 1);
            }
        } else {
            whole = _value;
        }
        ...
        string memory fractionStr = LibString.slice(
            LibString.toString(fraction),
            0,
            _precision
        );
...
library utils {
    ...
    function rgba(
        ...
        string memory formattedA = _a < 100
            ? string.concat("0.", LibString.toString(_a))
            : "1";
        return
            string.concat(
                "rgba(",
    ...
    function toString(
        ...
        require(
            precision <= digits && precision > 0,
            "Utils: precision invalid"
        );
