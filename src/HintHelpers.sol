    ...
    */
        ...
        returns (uint256 hintId, uint256 diff, uint256 latestRandomSeed)
    {
        ...
        for (uint256 i = 1; i < _numTrials; ++i) {
            ...
            uint256 currentId = troveManager.getTroveFromTroveIdsArray(
                arrayIndex
            );
    ...
    function _calcUpfrontFee(
        ...
        return
            (_debt * _avgInterestRate * UPFRONT_INTEREST_PERIOD) /
            ONE_YEAR /
            DECIMAL_PRECISION;
    }
    ...
    function predictOpenTroveUpfrontFee(
        ...
        openTrove.newWeightedRecordedDebt =
            openTrove.debtIncrease *
            _interestRate;

        uint256 avgInterestRate = activePool
            .getNewApproxAvgInterestRateFromTroveChange(openTrove);
    ...
    function predictAdjustInterestRateUpfrontFee(
        ...
        return
            _predictAdjustInterestRateUpfrontFee(
                activePool,
                trove,
                _newInterestRate
            );
    ...
    function _predictAdjustInterestRateUpfrontFee(
        ...
        uint256 avgInterestRate = _activePool
            .getNewApproxAvgInterestRateFromTroveChange(troveChange);
    ...
    function predictAdjustTroveUpfrontFee(
        ...
        (, , , , , , , , address batchManager, ) = troveManager.Troves(
            _troveId
        );
        ...
        if (batchManager == address(0)) {
            troveChange.newWeightedRecordedDebt =
                (trove.entireDebt + _debtIncrease) *
    ...
    function predictOpenTroveAndJoinBatchUpfrontFee(
        uint256 _collIndex,
        uint256 _borrowedAmount,
        address _batchAddress
    ) external view returns (uint256) {
        ...
        openTrove.newWeightedRecordedDebt =
            (batch.entireDebtWithoutRedistribution + _borrowedAmount) *
            batch.annualInterestRate;
    ...
    function predictJoinBatchInterestRateUpfrontFee(
        ...
        newBatchTroveChange.appliedRedistUSDXDebtGain = trove
            .redistUSDXDebtGain;
        newBatchTroveChange.batchAccruedManagementFee = batch
        ...
        uint256 avgInterestRate = activePool
            .getNewApproxAvgInterestRateFromTroveChange(newBatchTroveChange);
        return _calcUpfrontFee(trove.entireDebt, avgInterestRate);
    }

    function predictRemoveFromBatchUpfrontFee(
        uint256 _collIndex,
        uint256 _troveId,
        uint256 _newInterestRate
    ) external view returns (uint256) {
