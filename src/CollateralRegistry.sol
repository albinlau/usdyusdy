    ...
    ICollateralRegistry
{
    ...
    function redeemCollateral(
        ...
        if (totals.unbacked == 0) {
            ...
            // redemption (see CS-USDX-013). Instead, truncate the redemption to total unbacked. If this happens, the
            // redeemer can call `redeemCollateral()` a second time to redeem the remainder of their USDX.
            if (_usdxAmount > totals.unbacked) {
                _usdxAmount = totals.unbacked;
            }
        }
        ...
        for (uint256 index = 0; index < totals.numCollaterals; index++) {
            ...
            if (unbackedPortions[index] > 0) {
                uint256 redeemAmount = (_usdxAmount * unbackedPortions[index]) /
                    totals.unbacked;
                if (redeemAmount > 0) {
                    ITroveManager troveManager = getTroveManager(index);
                ...
                // Ensure that per-branch redeems add up to `_usdxAmount` exactly
                _usdxAmount -= redeemAmount;
    ...
    function _updateBaseRateAndGetRedemptionRate(
        uint256 _usdxAmount,
        uint256 _totalUSDXSupplyAtStart
    ) internal {
        uint256 newBaseRate = _getUpdatedBaseRateFromRedemption(
            _usdxAmount,
            _totalUSDXSupplyAtStart
        );
    ...
    function _getUpdatedBaseRateFromRedemption(
        ...
        uint256 _totalUSDXSupply
    ) internal view returns (uint256) {
        // decay the base rate
        uint256 decayedBaseRate = _calcDecayedBaseRate();

        // get the fraction of total supply that was redeemed
        uint256 redeemedUSDXFraction = (_redeemAmount * DECIMAL_PRECISION) /
            _totalUSDXSupply;
    ...
    function getEffectiveRedemptionFeeInUSDX(
        uint256 _redeemAmount
    ) external view override returns (uint256) {
        uint256 totalUSDXSupply = usdxToken.totalSupply();
        uint256 newBaseRate = _getUpdatedBaseRateFromRedemption(
